...
func _input(event):
	...
	if event is InputEventMouseMotion:
		if draging:
			...
			rotate_object_local(Vector3(0, 1, 0), (-mousepos.x/100)) # first rotate in Y
			rotate_object_local(Vector3(1, 0, 0), (-mousepos.y/100)) # then rotate in X
			rotation.z=0
